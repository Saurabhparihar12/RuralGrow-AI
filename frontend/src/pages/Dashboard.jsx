...
export default function Dashboard() {
  ...
  // Live Gemini AI review reply regeneration
  const handleRegenerateAI = async () => {
    if (!selectedReviewId) return;
    const selectedRev = reviews.find(r => r.id === selectedReviewId || r._id === selectedReviewId);
    ...
    try {
      const res = await fetch('http://localhost:5000/api/ai/review-reply', {
        ...
        body: JSON.stringify({
          author: selectedRev.author,
          shopName: selectedRev.shopName,
          reviewText: selectedRev.reviewText,
          rating: selectedRev.rating
        })
      });
      ...
      if (data.success) {
        ...
        if (data.source.includes('fallback') || data.source.includes('simulator')) {
          showToast('Draft updated using offline simulator.', 'info');
        } else {
          showToast('Draft regenerated using Gemini AI!', 'success');
        }
      } else {
        showToast(data.message || 'Failed to generate AI reply.', 'error');
      }
    } catch (err) {
      showToast('Could not connect to AI services.', 'error');
    } finally {
    ...
    try {
      ...
      const aiRes = await fetch('http://localhost:5000/api/ai/marketing-caption', {
        ...
        body: JSON.stringify({
          productName,
          shopType,
          details: `Organic product from local Uttarakhand ${shopType}`
        })
      });

      const aiData = await aiRes.json();
      if (aiData.success) {
        finalCaptionText = aiData.replyText;
        if (aiData.source.includes('fallback') || aiData.source.includes('simulator')) {
          showToast('Caption created using offline simulator.', 'info');
        } else {
          showToast('Instagram caption generated by Gemini AI!', 'success');
        }
      } else {
        throw new Error(aiData.message || 'Failed to generate AI caption.');
      }
      ...
      showToast(err.message || 'Error generating marketing caption.', 'error');
    } finally {
                          ...
                          >
                            ...
                            <div className="flex justify-end space-x-2">
                              <Button 
                                variant="primary" 
                                size="sm"
                                className="text-xs py-2 px-4 cursor-pointer bg-forest-900 dark:bg-clay-50 text-clay-50 dark:text-forest-950 border border-transparent"
                                isLoading={isAILoading}
                                onClick={handleRegenerateAI}
                              >
                                Re-compose with Gemini
                              </Button>
                              <Button 
                                variant="outline" 
                                size="sm"
                  ...
                  <div className="lg:col-span-5 bg-white dark:bg-[#19221F] border border-slate-200/50 dark:border-slate-800/40 rounded-3xl p-6 shadow-xs space-y-5">
                    <form onSubmit={handleGeneratePromo} className="space-y-5">
                      ...
                      <Button 
                        ...
                        disabled={user && user.role === 'visitor'}
                      >
                        {user && user.role === 'visitor' ? 'Viewer Role: Write Disabled' : 'Write Instagram Draft with AI'}
                      </Button>
                    </form>
